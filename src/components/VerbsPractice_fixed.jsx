...
const VerbsPractice = () => {
  ...
  useEffect(() => {
    const loadVerbs = async () => {
      try {
        ...
        // Load the additionalFrenchVerbs directly from the import
        ...
        // EMERGENCY FIX: If we have no verbs from the data service, use the direct import
        if (allVerbsRaw.length < 20) {
          console.warn('⚠️ Emergency fix activated: Loading verbs directly from additionalFrenchVerbs');
          const additionalFrenchVerbs = await import('../data/additionalFrenchVerbs');
          console.log('Direct import additionalFrenchVerbs:', additionalFrenchVerbs.default.length, 'verbs found');
          
          // Use the additionalFrenchVerbs as our fallback verbs
          if (additionalFrenchVerbs.default && additionalFrenchVerbs.default.length > 0) {
            // Process to ensure correct format
            const processedVerbs = additionalFrenchVerbs.default.map((verb, index) => {
              // Create a copy to avoid mutating the original
              const processedVerb = { ...verb };
              
              // Fix english value if needed
              if (!processedVerb.english || !processedVerb.english.startsWith('to ')) {
                console.log(`Fixing English for verb: ${processedVerb.id} - ${processedVerb.infinitive}`);
                processedVerb.english = `to ${processedVerb.infinitive || 'unknown'}`;
              }
              
              return processedVerb;
            });
            
            window.directLoadedVerbs = processedVerbs;
            console.log(`Successfully loaded ${window.directLoadedVerbs.length} verbs directly as fallback`);
          }
        }
        ...
        // Check if we have directly loaded verbs from emergency fix
        const useDirectVerbs = window.directLoadedVerbs && window.directLoadedVerbs.length > 0;
        
        if (allVerbsRaw.length > 0 || useDirectVerbs) {
          // Get the first verb - either from service or direct loaded verbs
          let verb;
          
          if (useDirectVerbs) {
            // Use directly loaded verbs if available
            console.log(`Using directly loaded verbs (${window.directLoadedVerbs.length} available)`);
            const randomIndex = Math.floor(Math.random() * window.directLoadedVerbs.length);
            verb = window.directLoadedVerbs[randomIndex];
          } else {
            // Use FrenchDataService as usual
            verb = FrenchDataService.getNextItem('verbs', userId);
          }
  ...
  const moveToNextVerb = () => {
    // Check if we're using directly loaded verbs
    const useDirectVerbs = window.directLoadedVerbs && window.directLoadedVerbs.length > 0;
    
    let nextVerb;
    
    if (useDirectVerbs) {
      // Use a random verb from the directly loaded verbs
      const randomIndex = Math.floor(Math.random() * window.directLoadedVerbs.length);
      nextVerb = window.directLoadedVerbs[randomIndex];
      console.log(`Getting next verb from directly loaded verbs (${window.directLoadedVerbs.length} available)`);
    } else {
      // Get next verb from FrenchDataService
