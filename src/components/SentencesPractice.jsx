...
const SentencesPractice = () => {
    ...
    const loadSentences = async () => {
      try {
        ...
        if (sentences.length > 0) {
          // Get the first sentence - FIXED: Adding await for async function call
          const sentence = await FrenchDataService.getNextItem('sentences', userId);
  ...
  const handleSubmit = async () => {
    ...
    if (showingWrongAnswer) {
      ...
      setExplanation('');
      const nextSentence = await FrenchDataService.getNextItem('sentences', userId);
